void Naive_VETO_Condition(Leaf_Structure* const TheLeafs, Inputs_Structure* const Inputs, Tree_Structure* AnaTree, Int_t const Event, Int_t const Counter, TRandom3* generator)
{
  // This function performs the actual VETO condition. There are multiple versions of this function
  // available. This one simply does VETO anything when the VETO fired. The leafs that should be filled by the VETO
  // condition are:
  ...
  // =============================================================================================
  // The purpose of this function is to VETO anything when the VETO fired! Hence we must first set all
  ...
  {
      // Apply the Naive VETO condition:
  ...
  // Notice that only unVETOed bars should be taken along. Only makes sense when there is no tracker:
