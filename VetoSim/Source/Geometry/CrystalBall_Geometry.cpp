...
void CrystalBall_Geometry()
{
  ...
  Double_t X_position = Doubles->GetBinContent(181);
  Double_t Y_position = Doubles->GetBinContent(182);
  Double_t Z_position = Doubles->GetBinContent(183);
  
  Double_t Rotation_X = Doubles->GetBinContent(184);
  Double_t Rotation_Y = Doubles->GetBinContent(185);
  Double_t Rotation_Z = Doubles->GetBinContent(186);
  ...
  TString geoFileName = "../../Data/Geometry/CrystalBall.geo.root";							// NOTE: Change by C. A. Douma!
  // --------------------------------------------------------------------------
...
cout << 3 << endl;
  ...
  globalrot->RotateX(Rotation_X);
  globalrot->RotateY(Rotation_Y);
  globalrot->RotateZ(Rotation_Z);
  TGeoCombiTrans* globaltrans = new TGeoCombiTrans("globaltrans",X_position,Y_position,Z_position,globalrot);
  globaltrans->RegisterYourself();

  // add the sphere as Mother Volume
  top->AddNode(pCBLogWorld, 0, globaltrans);
