...
void NEBULA_Geometry()												// NOTE: Change by C. A. Douma!
{
  // =================================================================================
  ...
  Int_t NBarsNeuLAND = 50*Integers->GetBinContent(2);
  Int_t Neu_Exists = Integers->GetBinContent(216);
  Int_t Index_Start = 0;
  if (Neu_Exists==1) {Index_Start = NBarsNeuLAND;}
  ...
  Double_t neuLAND_paddle_dimx   = 90.;    // half of the length [cm]
  Double_t neuLAND_paddle_dimy   = 5.9;   // half of the width [cm]
  Double_t neuLAND_paddle_dimz   = 5.9;   // half of the depth [cm]
  Double_t neuLAND_gap_dim       = 0.05;   // total detector depth [cm]
  Double_t neuLAND_wrapping1_dim = 0.02;   // thickness of wrapping material [cm]
  Double_t neuLAND_wrapping2_dim = 0.03;   // thickness of wrapping material [cm]
  Double_t neuLAND_Dist_Part = NEBULA_Dist;  // distance between two parts f NEBULA [cm].
  ...
  TString geoFileName = "../../Data/Geometry/NEBULA.geo.root";							// NOTE: Change by C. A. Douma!
  // --------------------------------------------------------------------------
  ...
  {
      for (Int_t n = 0; n<2; ++n) // Double-layer in one part
      {
          for (Int_t i = 0; i<30; ++i) // neutron counters within a single layer
          {
              // define the positions:
              tz = (((Int_t) Layer) + 0.5)*total_dimz*2.0 + ((Int_t) k)*neuLAND_Dist_Part;
              tx = 0.0;
              ty = -1.0*(((Int_t) i) - 14.5)*total_dimy*2.0;
