...
void GFI_Geometry()
{
  ...
  Double_t X_position = Doubles->GetBinContent(171);
  Double_t Y_position = Doubles->GetBinContent(172);
  Double_t Z_position = Doubles->GetBinContent(173);
  
  Double_t Rotation_X = Doubles->GetBinContent(174);
  Double_t Rotation_Y = Doubles->GetBinContent(175);
  Double_t Rotation_Z = Doubles->GetBinContent(176);
  
  Double_t Module_Distance = Doubles->GetBinContent(177);
  ...
  TString geoFileName = "../../Data/Geometry/GFI.geo.root";									// NOTE: Change by C. A. Douma!
  // --------------------------------------------------------------------------
  ...
  pMatrix3->RotateX(Rotation_X);
  pMatrix3->RotateY(Rotation_Y);
  pMatrix3->RotateZ(Rotation_Z);
  TGeoCombiTrans* pMatrix2 = new TGeoCombiTrans("",X_position,Y_position,Z_position,pMatrix3);
  ...
  //TGeoCombiTrans*
  //Matrix4 = new TGeoCombiTrans("", dx,dy,dz,pMatrix5);
  
  TVector3 pos;
  pos.SetX(X_position);
  pos.SetY(Y_position);
  pos.SetZ(Z_position);
  pos.RotateZ(-1.0*(TMath::Pi()/180.0)*Rotation_Z);
  pos.RotateY(-1.0*(TMath::Pi()/180.0)*Rotation_Y);
  pos.RotateX(-1.0*(TMath::Pi()/180.0)*Rotation_X);
  pos.SetZ(pos.Z()+Module_Distance);
  pos.RotateX((TMath::Pi()/180.0)*Rotation_X);
  pos.RotateY((TMath::Pi()/180.0)*Rotation_Y);
  pos.RotateZ((TMath::Pi()/180.0)*Rotation_Z);
  
  TGeoRotation *pMatrix5 = new TGeoRotation("");
  pMatrix5->RotateX(Rotation_X);
  pMatrix5->RotateY(Rotation_Y);
  pMatrix5->RotateZ(Rotation_Z);
  TGeoCombiTrans* pMatrix4 = new TGeoCombiTrans("",pos.X(),pos.Y(),pos.Z(),pMatrix5);
  ...
  TGeoRotation* globalrot = new TGeoRotation("globalrot");                                                              // NOTE: This is changed by C. A. Douma.
  globalrot->RotateX(0.0);
  globalrot->RotateY(0.0);
  globalrot->RotateZ(0.0);
  TGeoCombiTrans* globaltrans = new TGeoCombiTrans("globaltrans",0.0,0.0,0.0,globalrot);
  globaltrans->RegisterYourself();
  
  pWorld->AddNode(pw1, 0, globaltrans);
