...
void VETOedFiredBars_Geometry()											// NOTE: Change by C. A. Douma!
{
  // =================================================================================
  ...
  Int_t PDG_Select = Integers->GetBinContent(406);
...
Counter = 8;
  ...
  // NOTE: Now we must only add the bars to the CELL that fired. Hence we first need to
  ...
  Long64_t nBytes = AnaLeafs->TheTree->GetEntry(Event);							        
  ...
  Bool_t fired = kFALSE;									       		
  Int_t NBarsFired = AnaLeafs->NbarsFired_NeuLAND->GetValue(0);		
  Int_t Nprims = AnaLeafs->Nprims_NeuLAND->GetValue(0);
  Int_t BarNr = 0;									        		
  Int_t ThisPDG = 0;	
  Int_t ThisTrackID = 0;
  Int_t checkTrackID = 0;
  Int_t checkPDG = 0;
  ...
  for(tz = -neuLAND_depth_dim + total_dimz; tz < neuLAND_depth_dim; tz += total_dimz*2) {
    ...
    for (ty = -total_dimx + total_dimy; ty < total_dimx; ty += total_dimy*2) {
      ...
      // check if this bar fired and if so, to which primary particle it belongs:			        
      for (Int_t loop = 0; loop<NBarsFired; ++loop)								
      {									        				
	BarNr = AnaLeafs->Neu_BarNumber_Fired->GetValue(loop);							
	ThisTrackID = AnaLeafs->Neu_Primary_TrackID->GetValue(loop);
	
	// check to which PDG it belongs:
	for (Int_t primloop = 0; primloop<Nprims; ++primloop)
	{
	  checkTrackID = AnaLeafs->Prim_TrackID->GetValue(primloop);
	  if (checkTrackID==ThisTrackID) {checkPDG = AnaLeafs->Prim_PDG->GetValue(primloop);}
	}
	
	if (BarNr==nindex) {fired = kTRUE; ThisPDG = checkPDG;}									
      }									        				
      
      // Only add the bar if it fired and if the PDG is right:						        
      if ((fired)&&((PDG_Select==ThisPDG)||(PDG_Select==0))&&(!(AnaLeafs->AllBars_IsVETOed->GetValue(nindex-1))))						        
      {										                                
      if (i % 2 == 1) {
  ...
  // The cell cannot be non-empty:			                                	                
  cell->AddNode(aLand, 1, new TGeoCombiTrans(0.0, 0.0, 2.0*neuLAND_depth_dim, zeroRotation));			
  // ========================================================================================================	// NOTE: Change by C. A. Douma!
