...
TCanvas* DrawHistDouma(TH1D* const Raw, TString const Title, TString const xlabel, TString const ylabel, Int_t const Color)
{
  ...
  Raw->SetMinimum(-0.2*max); // must be positive for log-scale!
  //Raw->SetMinimum(0.3);
  ...
  gStyle->SetHistLineWidth(2.0); // line thickness of histogram
