...
TCanvas* DrawGraphDouma(TGraphErrors* const Raw, TString const Title, TString const xlabel, TString const ylabel, Int_t const Color)
{
  ...
  Raw->SetMinimum(Ymin);
  Raw->SetMaximum(Ymax+0.1*Ylength);
  
  (Raw->GetYaxis())->SetLimits(0.0,Ymax+0.2*Ylength);
  ...
  Raw->SetLineWidth(2); // Set the thickness of the graph
