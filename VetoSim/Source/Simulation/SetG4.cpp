...
void SetG4()
{
    // We read the inputs-file and re-set this file to a TGeant4 MC Engine.
    ...
    TObjString* Monte_Carlo_Engine = new TObjString("TGeant4");
    Integers->SetBinContent(249,1); // So that we do not execute digitizers, etc with G4.
    Integers->SetBinContent(204,0); // So that our initial G4 run only simulates the nuclear collision, nothing else!
...
{
  TApplication* TheApp = new TApplication("SetG4",&argc,argv);
  SetG4();
  return 0;
}
#endif
