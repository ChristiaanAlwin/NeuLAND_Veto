...
void VETO_Digitizer()																	// NOTE: Change by C. A. Douma
{
  ...
  Double_t Calibration_Cboundary = ((Int_t) (Nneutrons+Nprotons))*(BeamE_d/40.0);
  Double_t Normal_Eboundary = 0.7*BeamE_d*((Int_t) Nparticles);
  Double_t Normal_Cboundary = ((Int_t) Nparticles)*(BeamE_d/40.0);
  ...
  Bool_t VETO = ((VETO_Exists==1)||(VETO_Exists==2));
  ...
  if (CalibrationID==1)																	// NOTE: Change by C. A. Douma
  {  
    out_file = "../../Data/Digitizer/VETO_" + Output_Name;
  }
  else
  {
    out_file = "../../Data/Digitizer/" + Output_Name + "_run=" + Nr_Run_str + "_VETO.root";
  }
  ...
  if (VETO_Exists==2)
  {
    R3BNveto_segDigitizer* Digitizer = new R3BNveto_segDigitizer(); // NOTE: VETO-copy of Jan Mayer his NeuLAND Digitizer. Notice that this does NOT have a UseBeam-function!
    run->AddTask(Digitizer);
  
    R3BNveto_segDigiMon* DigiMonitor = new R3BNveto_segDigiMon(); // NOTE: VETO-copy of Jan Mayer his Monitior of the Digitizer
    run->AddTask(DigiMonitor);
  
    R3BNveto_segClusterFinder* ClusterFinder = new R3BNveto_segClusterFinder(); // NOTE: VETO-copy of The NeuLAND Clusterfinder
    run->AddTask(ClusterFinder);
  }
  else
  {
    R3BNveto_segDigitizrOld* Digitizer = new R3BNveto_segDigitizrOld(); // NOTE: VETO-copy of Jan Mayer his NeuLAND Digitizer. Notice that this does NOT have a UseBeam-function!
    run->AddTask(Digitizer);
  
    R3BNVeto_segClusterFinder* ClusterFinder = new R3BNVeto_segClusterFinder(0); // NOTE: VETO-copy of The NeuLAND Clusterfinder
    ClusterFinder->SetSpaceRadius(ClusterSpaceRadius);
    ClusterFinder->SetTimeRadius(ClusterTimeRadius);
    run->AddTask(ClusterFinder);
  
    R3BNeutronCalib2D_seg* Calibrator = new R3BNeutronCalib2D_seg(); // NOTE: VETO-copy of The Pre-calibration of the 2D cuts on the number of Neutrons. It requires the UseBeam BeamE!
  ...
  else
  {
    cout << "### NOTE: The digitizer can only run with Geant3 as Monte Carlo Engine\n"
         << "### NOTE: and with a VETO detector in the setup!\n"
         << "### NOTE: You selected a different Monte Carlo Engine,\n"
	 << "### NOTE: or you eliminated the VETO from the setup!\n"
