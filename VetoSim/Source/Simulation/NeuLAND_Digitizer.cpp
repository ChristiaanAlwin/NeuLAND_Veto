...
void NeuLAND_Digitizer()																// NOTE: Change by C. A. Douma
{
  ...
  if (NeuLANDClass==2)
  {
    ...
    R3BNeuLANDClusterFindr* ClusterFinder = new R3BNeuLANDClusterFindr();
    ClusterFinder->SetSpaceRadius(ClusterSpaceRadius);       // Sets the cluster space radius to 7.5 cm.
    ClusterFinder->SetTimeRadius(ClusterTimeRadius);        // Sets the cluster time radius to 1.0 ns.
    ClusterFinder->SetTopologyBoundary(5.0);  // Sets the topological boundary equal to 5.0 cm.
    if (CalibrationID==1)
    {  
        ClusterFinder->SetHistograms(Calibration_Eboundary,(Int_t) Calibration_Cboundary);
    }
    else
    {  
        ClusterFinder->SetHistograms(Normal_Eboundary,(Int_t) Normal_Cboundary);
    }
    run->AddTask(ClusterFinder);
  }
  else
  {
    R3BNeulandDigitizrOld* Digitizer = new R3BNeulandDigitizrOld(); // Jan Mayer his NeuLAND Digitizer. Notice that this does NOT have a UseBeam-function!
    run->AddTask(Digitizer);
