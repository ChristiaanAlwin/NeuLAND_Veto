...
Select = 1;
    ...
    Double_t** Arrays = new Double_t*[12];
    Double_t** Errors = new Double_t*[12];
    ...
    for (Int_t n = 0; n<Nruns; ++n)
    {
        ...
        else {Run = VETOed1Run(n+1,Series,Select);}
    ...
    TGraphErrors** TheGraphs = new TGraphErrors*[13];
    for (Int_t k = 0; k<12; ++k)
    {
    ...
    TheGraphs[12] = new TGraphErrors(Nruns,XArray,Diff,Err_XArray,Err_Diff);
    TheGraphs[12]->SetMaximum(max);
    TheGraphs[12]->SetMinimum(0.0);
    ...
    Legenda[11] = "Good Events";
    Legenda[12] = "Protons-Neutrons";
    
    TPaveText** text = new TPaveText*[13];
    ...
    {   
      ...
      if (k==11) {text[k]->SetTextColor(1);}
      if (k==12) {text[k]->SetTextColor(43);}
