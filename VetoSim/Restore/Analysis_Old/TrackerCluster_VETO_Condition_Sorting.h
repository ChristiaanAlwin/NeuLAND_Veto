...
void TrackerCluster_VETO_Condition(Leaf_Structure* const TheLeafs, Inputs_Structure* const Inputs, Tree_Structure* AnaTree, Int_t const Event, Int_t const Counter, TRandom3* generator)
{
  ...
  if (TheLeafs->Tracker_Exists)
  {
      ...
      std::vector<Int_t> ClusNo;
      std::vector<Double_t> zdiff;
      ...
      if ((NbarsFired>0)&&(Nclusters>0)&&(Nvertex>0))
      {
          for (Int_t kveto = 0; kveto<NbarsFired; ++kveto)
          {
              ...
              for (Int_t kcluster = 0; kcluster<Nclusters; ++kcluster)
              {
                  ...
                  if (AnaTree->NeuLAND_ClusterSize[kcluster]>1)
                  {
                    ...
                    if (BarOfLine==VETObarNr)
                    {
                      ...
                      {
                          // then we already found at least one cluster. Then check how good
                          // this one is. We check on its z starting position.
                          // Hence transform x,y,z to the NeuLAND standard frame.
                          // This check must be applied for all clusters found so far.
                          
                          // so loop over all clusters found so far:
                          for (Int_t k = 0; k<NClustersThroughBar; ++k)
                          {
                            // Transform to the NeuLAND standard frame:
                            ...
                            PrevPoint.SetX(AnaTree->NeuLAND_ClusterX[ClusNo.at(k)] - Inputs->NeuLAND_geo_Xpos);
                            PrevPoint.SetY(AnaTree->NeuLAND_ClusterY[ClusNo.at(k)] - Inputs->NeuLAND_geo_Ypos);
                            PrevPoint.SetZ(AnaTree->NeuLAND_ClusterZ[ClusNo.at(k)] - Inputs->NeuLAND_geo_Zpos);
                            ...
                            // compute difference in z between this cluster and all 
                            // previously found clusters.
                            zdiff.push_back(ThisPoint.Z() - PrevPoint.Z());
                          }
                          
                          // So for previosuly found clusters where zdiff is positive, that previous
                          // cluster has a smaller z that this one, hence the previous one is better.
                          
                          // So now we want to insert the new cluster in the ClusNo-vector in
                          // such a way that the order of the vectors represents the quality of
                          // the clusters. index 0 should be the best one and so o.
                          
                          // Hence loop from back to front in the vectors. First add the new cluster
                          // at the end.
                          ClusNo.push_back(kcluster);
                          
                          // then loop:
                          for (Int_t k = 0; k<NClustersThroughBar; ++k)
                          {
                              // We go from back to front. Now if zdiff(n-k-1)<0, then the new
                              // cluster is better then cluster ClusNo(n-k-1), Hence
                              // ClusNo(n-k-1) is moved to ClusNo(n-k) and ClusNo(n-k-1)
                              // becomes the new cluster. Due to looping backward, we know
                              // that at any point in the loop, ClusNo(n-k) is already kcluster.
                              // hence the switch is easy. By doing this consistently,
                              // the order will build up automatically.
                              
                              // So check and switch:
                              if (zdiff.at(NClustersThroughBar - k - 1)<0)
                              {
                                  ClusNo.at(NClustersThroughBar - k) = ClusNo.at(NClustersThroughBar - k - 1);
                                  ClusNo.at(NClustersThroughBar - k - 1) = kcluster;
                              }
                              
                              // Done!
                          }
                          
                          // So now we added the cluster to the ClusNo-vector. Now update the counter:
                          NClustersThroughBar = NClustersThroughBar + 1;
                          
                          // Done! the vector is already updated & and the boolian is true already.
                      }
                      else   
                      {
              ...
              // the vertex lying closest to this line should be VETOed. But only if TOF works. Hence we should do a loop
              // over all found clusters (in order of importantce!!!). Then we simply try the next one if the first fails.
              succes = kFALSE;
              
              for (Int_t kclus = 0; kclus<NClustersThroughBar; ++kclus)
              {
                  if (!succes) // we only loop as long as there is nothing VETOed yet. Then we try all the clusters untill we find the right one!
                  {              
              ...
              {
                  // Load the startpoint:
                  StartPoint.SetX(AnaTree->NeuLAND_ClusterX[ClusNo.at(kclus)]);
                  StartPoint.SetY(AnaTree->NeuLAND_ClusterY[ClusNo.at(kclus)]);
                  StartPoint.SetZ(AnaTree->NeuLAND_ClusterZ[ClusNo.at(kclus)]);
                  tstart = AnaTree->NeuLAND_ClusterTime[ClusNo.at(kclus)];
                  
                  // load the stop point:
                  StopPoint.SetX(AnaTree->NeuLAND_CStopX[ClusNo.at(kclus)]);
                  StopPoint.SetY(AnaTree->NeuLAND_CStopY[ClusNo.at(kclus)]);
                  StopPoint.SetZ(AnaTree->NeuLAND_CStopZ[ClusNo.at(kclus)]);
                  tstop = AnaTree->NeuLAND_CStopTime[ClusNo.at(kclus)];
              ...
              if (AnaTree->T_Reconstructed_PrimPoints[kmin]>VETObarTOF) {AnaTree->Reconstructed_PrimPoints_IsVETOed[kmin] = kTRUE; succes = kTRUE;} // NOTE: Put this equal to kTRUE to end the cluster loop!
              
              // Then now end our loop over suitable clusters:
                  }
              }
