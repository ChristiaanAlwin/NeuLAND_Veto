...
sed -i '280 a add_subdirectory (veto_seg)' ./CMakeLists.txt
sed -i '283 a add_subdirectory (Nveto_seg)' ./CMakeLists.txt
...
sed -i -e "s:kLUMON:kLUMON,kVETO_SEG:" ./r3bdata/R3BDetectorList.h
sed -i -e "s:kLuMonPoint:kLuMonPoint,kveto_segPoint:" ./r3bdata/R3BDetectorList.h
...
sed -i -e 's:<<endl;: << ", VETO_SEG "<<GetNPoints(kVETO_SEG)<<endl;:' ./r3bdata/R3BMCTrack.cxx
sed -i '138 a else if ( detId == kVETO_SEG  ) return ( (fNPoints & ( 1 <<32 ) ) >> 32);' ./r3bdata/R3BMCTrack.cxx
sed -i '235 a    else if ( iDet == kVETO_SEG ) {' ./r3bdata/R3BMCTrack.cxx
sed -i '236 a      if      ( nPoints < 0 ) nPoints = 0;' ./r3bdata/R3BMCTrack.cxx
sed -i '237 a      else if ( nPoints > 1 ) nPoints = 1;' ./r3bdata/R3BMCTrack.cxx
sed -i '238 a      fNPoints = ( fNPoints & ( ~ (  1 << 32 ) ) )  |  ( nPoints << 32 );' ./r3bdata/R3BMCTrack.cxx
sed -i '239 a    }' ./r3bdata/R3BMCTrack.cxx
