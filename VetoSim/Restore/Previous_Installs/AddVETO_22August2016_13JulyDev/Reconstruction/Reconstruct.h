void R3BNeuLANDTracker::Reconstruct()
{
  ...
  R3BNeuLANDCluster* cluster1;
  R3BNeuLANDCluster* cluster2;
  ...
  for (Int_t k1 = 0; k1<NClusters; ++k1)
  {
      ...
      cluster1 = (R3BNeuLANDCluster*) fArrayClusters->At(k1);
      ...
      if ((cluster1->GetSize()>1)&&(!(cluster1->IsEliminated())))
      {
          ...
          for (Int_t k2 = k1+1; k2<NClusters; ++k2)
          {
              ...
              if (!WeMarked)
              {
                    ...
                    cluster2 = (R3BNeuLANDCluster*) fArrayClusters->At(k2);
  ...
  {
      // load the cluster:
      cluster1 = (R3BNeuLANDCluster*) fArrayClusters->At(k);
  ...
  if (fNClustersSorted>0)
  {
    ...
    for (Int_t kneutron = 0; kneutron<fNeutronNumber; ++kneutron)
    {
        ...
        {
            // Load the cluster:
            cluster1 = (R3BNeuLANDCluster*) fVectorClusters.at(kcluster);
