...
sed -i '9 a Reconstruction/R3BNeuLANDCluster.cxx' ./CMakeLists.txt
sed -i '10 a Reconstruction/R3BNeuLANDVertex.cxx' ./CMakeLists.txt
sed -i '11 a Reconstruction/R3BNeuLANDClusterFinder.cxx' ./CMakeLists.txt
sed -i '12 a Reconstruction/R3BNeuLANDTracker.cxx' ./CMakeLists.txt
sed -i '40 a ${R3BROOT_SOURCE_DIR}/neuland/Reconstruction' ./CMakeLists.txt
sed -i '41 a ${R3BROOT_SOURCE_DIR}/r3bdata/veto_segData' ./CMakeLists.txt

sed -i '15 a #pragma link C++ class R3BNeuLANDCluster+;' ./NeulandLinkDef.h
sed -i '16 a #pragma link C++ class R3BNeuLANDVertex+;' ./NeulandLinkDef.h
sed -i '17 a #pragma link C++ class R3BNeuLANDClusterFinder+;' ./NeulandLinkDef.h
