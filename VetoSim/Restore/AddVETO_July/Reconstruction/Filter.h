void R3BNeuLANDTracker::Filter()
{
  ...
  if (NvetoDigis>0)
  {
      ...
      {
          // load the VETO hit:
          VetoDigi = (R3Bveto_segDigi*) fArrayVetoDigis->At(kveto);
          ...
          {
              // Load the cluster:
              cluster = (R3BNeuLANDCluster*) fArrayClusters->At(kc);
              ...
              {     
                  // Load the hits:
                  veto_t = VetoDigi->GetTdc();
                  veto_x = VetoDigi->GetXX();
                  veto_y = VetoDigi->GetYY();
                  veto_z = VetoDigi->GetZZ();
          ...
          if (MinDistIndex>-1)
          {
              cluster = (R3BNeuLANDCluster*) fArrayClusters->At(MinDistIndex);
              cluster->Eliminate();
          }
  ...
  if (fEliminationLevel>0)
  {
      ...
      for (Int_t kel = 0; kel<fEliminationLevel; ++kel)
      {
          ...
          for (Int_t k1 = 0; k1<NClusters; ++k1)
          {
              ...
              ELIMcluster = (R3BNeuLANDCluster*) fArrayClusters->At(k1);
              ...
              if (ELIMcluster->IsEliminated())
              {
                  ...
                  for (Int_t k2 = 0; k2<NClusters; ++k2)
                  {
                      ...
                      if (k2!=k1)
                      {
                          ...
                          cluster = (R3BNeuLANDCluster*) fArrayClusters->At(k2);
                  ...
                  if (kMin>-1)
                  {
                    cluster = (R3BNeuLANDCluster*) fArrayClusters->At(kMin);
                    cluster->MarkEl();
                  }
