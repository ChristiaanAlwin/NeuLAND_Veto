Bool_t R3BNeuLANDTracker::IsElastic(R3BNeuLANDCluster *c1, R3BNeuLANDCluster *c2)
{
  //    Purpose of this function: search for scattering
  ...
  // Set parameters for the elastic search:
  Double_t c = 29.9792458;
  Double_t dio = 10.6; // 3 times half the diogonal of a paddle
  Double_t amu = 931.494028;
    
  // Compute the time correction from the beam:
  Double_t Beam_TravelTime = TMath::Sqrt((fTarget_Xpos - fBeam_Xpos)*(fTarget_Xpos - fBeam_Xpos) + (fTarget_Ypos - fBeam_Ypos)*(fTarget_Ypos - fBeam_Ypos) + (fTarget_Zpos - fBeam_Zpos)*(fTarget_Zpos - fBeam_Zpos));
  Beam_TravelTime = Beam_TravelTime/(fBeamBeta*c);

  // Declare some varibales that we need:
  Double_t Ma,Mb,Mc,Md,Ka,Thc,Ei,Pa,AA,BB,a,b,cc,Pc1,Pc2;
  Double_t Pd1,Thd1,Ec1,Ed1,Kc1,Kd1,Qsqr1;
  Double_t v4xmin,v4ymin,v4zmin;
  Double_t v3xmin,v3ymin,v3zmin;
  TVector3 pos1;
  TVector3 pos2;

  // Load data on particle 1: The incoming neutron. (at angle zero, we assume!)
  c1->StartPosition(pos1);
  Double_t v1x=pos1.X()-fTarget_Xpos;
  Double_t v1y=pos1.Y()-fTarget_Ypos;
  Double_t v1z=pos1.Z()-fTarget_Zpos;
  Double_t dt=c1->GetStartT()-Beam_TravelTime;	 
  ...
  // Load data on particle 2. This is unneeded, since it is at rest. We know what happens.
  
  // Load the date on particle 4: the scattered proton. We do this first, since it is
  // assumed that the scattered neutron does not fire bars, but the proton does!
  ...
  Double_t theta4Measured=TMath::ACos((v1x*v4x + v1y*v4y + v1z*v4z )/TMath::Sqrt(v1x*v1x + v1y*v1y + v1z*v1z)/sqrt(v4x*v4x + v4y*v4y + v4z*v4z));		
  Double_t theta4Measuredmin=theta4Measured;
  Double_t theta4Measuredmax=theta4Measured;
  
  if ((v4x*v4x + v4y*v4y + v4z*v4z)==0.0)
  {
    theta4Measured=1.55;
    theta4Measuredmin=0.;
  ...
  for (Int_t k = 1; k<3; ++k)
  {
	...
	tempAngle = TMath::ACos((v1x*v4xmin + v1y*v4ymin + v1z*v4zmin)/TMath::Sqrt(v1x*v1x + v1y*v1y + v1z*v1z)/TMath::Sqrt(v4xmin*v4xmin + v4ymin*v4ymin + v4zmin*v4zmin));
  ...
  // Now finally, we do the computations on particle 3: the scattered neutron.
  TVector3 pos3;
  ...
  Double_t theta56=TMath::ACos((v5x*v6x + v5y*v6y + v5z*v6z)/TMath::Sqrt(v5x*v5x + v5y*v5y + v5z*v5z)/sqrt(v6x*v6x + v6y*v6y + v6z*v6z));		     
  
  if (((v5x*v5x + v5y*v5y + v5z*v5z)==0.0)||((v6x*v6x + v6y*v6y + v6z*v6z)==0.0))
  {
    theta56=3.14;
  }
  
  Double_t theta3=TMath::ACos((v1x*v3x + v1y*v3y + v1z*v3z)/TMath::Sqrt(v1x*v1x + v1y*v1y + v1z*v1z)/sqrt(v3x*v3x + v3y*v3y + v3z*v3z));		     
  ...
  for (Int_t k = 1; k<3; ++k)
  {
	...
	tempAngle=TMath::ACos((v1x*v3xmin + v1y*v3ymin + v1z*v3zmin)/TMath::Sqrt(v1x*v1x + v1y*v1y + v1z*v1z)/TMath::Sqrt(v3xmin*v3xmin + v3ymin*v3ymin + v3zmin*v3zmin));
  ...
  // Now after we laoded all the data, we can do the actual computation!
  ...
  // Do something with some variables to avoid warnings:
  Pc2 = 0.0;
  Qsqr1 = 0.0 + Pc2;
  Pc2 = Pc2 + Qsqr1;
  
  // Finally: decide if Cluster comes from scattered neutron or another neutron is needed!
  Double_t protonEnergy = 8.76839 + 4.13858*(c1->GetE()) - 0.00337368*(c1->GetE())*(c1->GetE());
 
  if ((p3bmax>p3min)&&(p3bmin<p3max)&&(theta4max>theta4Measuredmin)&&(theta4min<theta4Measuredmax)&&(K4bmax>protonEnergy)&&(K4bmin<protonEnergy)&&(theta56*180./3.14>120.)) 
  {
    return kTRUE;
  } else {
    return kFALSE;
