...
void R3BNeuLANDClusterFinder::Exec(Option_t *option)
{
  ...
  {
    digi = (R3BLandDigi*) fArrayDigi->At(k);
  ...
  {  
    for (Int_t k1 = 0; k1<nDigis; ++k1)
    {
      ...
      {
          // load the digis:
          digi1 = (R3BLandDigi*) fVectorDigi.at(k1);
          digi2 = (R3BLandDigi*) fVectorDigi.at(k2);
	  ...
	  if ((rad<fSpaceRadius)&&(TMath::Abs(post1-post2)<fTimeRadius)&&(k1!=k2)) // if k1==k2 it is the same poit --> pointless!
          {
              ...
              if ((belongsToCluster[k1]==-1)&&(belongsToCluster[k2]==-1))
              {
                  ...
                  new ((*fArrayCluster)[nClusters]) R3BNeuLANDCluster(digi1,digi2);
              ...
              else if ((belongsToCluster[k1]>-1)&&(belongsToCluster[k2]==-1))
              {
                  ...
                  // update the cluster:
                  Cluster = (R3BNeuLANDCluster*) fArrayCluster->At(clusNo1);
              ...
              else if ((belongsToCluster[k1]==-1)&&(belongsToCluster[k2]>-1))
              {
                  ...
                  // update the cluster:
                  Cluster = (R3BNeuLANDCluster*) fArrayCluster->At(clusNo2);
              ...
              else if ((belongsToCluster[k1]>-1)&&(belongsToCluster[k2]>-1)&&(belongsToCluster[k1]!=belongsToCluster[k2]))
              {
                ...
                Cluster = (R3BNeuLANDCluster*) fArrayCluster->At(clusNo1);
                Cluster->Merge((R3BNeuLANDCluster*) fArrayCluster->At(clusNo2),fArrayDigi);
                
                // And now we must eliminate the second cluster, by replacing
                // it with the end cluster in the array:
                Cluster = (R3BNeuLANDCluster*) fArrayCluster->At(clusNo2);
                Cluster->Replace((R3BNeuLANDCluster*) fArrayCluster->At(nClusters-1)); // NOTE: Need n-1 since nClusters is not a C++ index!
  ...
  if ((nDigis>0)&&(nDigis<Nbars))
  {
    for (Int_t i = 0; i<nDigis; i++) 
    {
      ...
      digi1 = (R3BLandDigi*) fVectorDigi.at(i);
	...
	rad = TMath::Abs(posx1-posx2);
      ...
      if (NoNeighbours)
      {
          ...
          new ((*fArrayCluster)[nClusters]) R3BNeuLANDCluster(digi1);
