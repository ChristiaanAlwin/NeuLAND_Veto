...
void R3BNeutronTracker2D::ReadCalibrFiles(TString const Directory)
{
  ...
  for (Int_t k = 0; k<(fMaxProtons+1); ++k)
  {
      ...
      kstr = st.Itoa(k,10);
      Name = Directory + "/Energy_Cuts_" + kstr + "p.txt";
...
void R3BNeutronTracker2D::Exec(Option_t* opt)
{
  ...
  // Loop over veto digis. Notice that if the VETO digitizer is not supplied,
  // one will always use the 0 protons case!
  fvetoTOFhits = 0;
  if (fveto_segDigi!=0)
  {
