...
# Also implement the time cuts to the NeuLAND digitizer:
sed -i '56 a Double_t fTcut_up;' ./R3BNveto_segDigitizer.h
sed -i '57 a Double_t fTcut_low;' ./R3BNveto_segDigitizer.h

sed -i '110 a fTcut_up = Doubles->GetBinContent(344);' ./R3BNveto_segDigitizer.cxx
sed -i '111 a fTcut_low = Doubles->GetBinContent(345);' ./R3BNveto_segDigitizer.cxx
sed -i "174 a if ((veto_segPoint->GetEnergyLoss() > 0. && media == 3)&&(veto_segPoint->GetTime()<fTcut_up)&&(veto_segPoint->GetTime()>fTcut_low)) {" ./R3BNveto_segDigitizer.cxx
sed -i '174 d' ./R3BNveto_segDigitizer.cxx
...
# Also implement the time cuts to the NeuLAND digitizer:
sed -i '56 a Double_t fTcut_up;' ./R3BNeulandDigitizer.h
sed -i '57 a Double_t fTcut_low;' ./R3BNeulandDigitizer.h

sed -i '113 a fTcut_up = Doubles->GetBinContent(344);' ./R3BNeulandDigitizer.cxx
sed -i '114 a fTcut_low = Doubles->GetBinContent(345);' ./R3BNeulandDigitizer.cxx
sed -i "172 a if ((landPoint->GetEnergyLoss() > 0. && media == 3)&&(landPoint->GetTime()<fTcut_up)&&(landPoint->GetTime()>fTcut_low)) {" ./R3BNeulandDigitizer.cxx
sed -i '172 d' ./R3BNeulandDigitizer.cxx
