...
class R3BNeutronTracker2D : public FairTask
{
 public:
  ...
  virtual void Reset();
    ...
    inline void DisablePrimParts() {fUsePrimParts = kFALSE;}
    inline void EnablePrimParts() {fUsePrimParts = kTRUE;}

  void UseBeam(Double_t beam_energy, Double_t beam_beta);
  
void ReadCalibrFiles();
...
Int_t fCurrentProtons;
  ...
  Double_t fneuland_Global_Xpos;
  Double_t fneuland_Global_Ypos;
  Double_t fneuland_Global_Zpos;
  
  Double_t fneuland_Global_XRotation;
  Double_t fneuland_Global_YRotation;
  Double_t fneuland_Global_ZRotation;
  
  Int_t fneuland_NSinglePlanes;
