...
fveto_segDigi = (TClonesArray*) ioman->GetObject("veto_segDigi");
  ...
  // Connect to inputs and load the neuland geometry:
...
void R3BNeutronTracker2D::Exec(Option_t* opt)
{
  ...
  // Loop over digis to compute NeuLAND TOF:
  fNeuLAND_TOF = 1e99;
  sumTotalEnergy=0;   
  R3BLandDigi *digi;
  ...
  // Determine amount of bars firing before the NeuLAND TOF:
  fvetoTOFhits = 0;
  ...
  // Now we begin with creating neuland first hits for each cluster
  // inside NeuLANDs first single plane. Declare what we need:
  TVector3 Pos;
  ...
  {
      // Obtain the clusters:
      cluster =  (R3BNeuLandCluster*) fArrayCluster->At(k);
  
      // Transform coordinates to NeuLAND frame:
      Pos.SetX(cluster->GetStartX() - fneuland_Global_Xpos);
      Pos.SetY(cluster->GetStartY() - fneuland_Global_Ypos);
      Pos.SetZ(cluster->GetStartZ() - fneuland_Global_Zpos);
        
      Pos.RotateZ(-1.0*fneuland_Global_ZRotation);
      Pos.RotateY(-1.0*fneuland_Global_YRotation);
      Pos.RotateX(-1.0*fneuland_Global_XRotation);
        
      // Determine if it is in plane 1:
      Zpoint = Pos.Z() + 0.5*5.0*((Int_t) fneuland_NSinglePlanes);
      
      // decide:
      if ((Zpoint>0.0)&&(Zpoint<5.0))
      {
          // then it is in the first plane. Hence now create a NeutHit
          // no matter what:
          new ((*fNeutHits)[fNofTracks]) R3BNeutHit(cluster->GetStartX(),cluster->GetStartY(),cluster->GetStartZ(),cluster->GetStartT());
          fNofTracks = fNofTracks + 1;
          
          // Count the energy in these clusters:
          Edep = Edep + cluster->GetE();
          
          // The number of clusters is counted by fNofTracks.
          
          // Eliminate these clusters from the search by marking them:
          cluster->Mark();
      }
  ...
  // NOTE: we already did the charged particles, so we need to apply
  // always the cuts for 0 hits:
  fCurrentProtons = 0;
  ...
  if (f2DCutEnabled)
  {
    ...
    // Apply the correction for the clusters & energy:
    fdClusters = fdClusters - fNofTracks;
    fEtot = fEtot - Edep;
    
    // do the cuts for the remainders of the neutrons:
  ...
  // NOTE: Now do the real algorithm:   
  ...
  CalculateMassInv();
//  if (fUsePrimParts==kTRUE) {CalculateExce();} // NOTE: CalculateExce() cannout run without the primparts!
