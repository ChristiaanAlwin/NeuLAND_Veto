...
do
  ...
  if [[ ${Array[${n}]} == *"PaddleTamex"* ]]
  then 
    echo "${Array[${n}]}" > ./Safe/FileLine.txt
    sed -i -e "s:PaddleTamex:Paddle_segTamex:" ./Safe/FileLine.txt
    read TheLine < ./Safe/FileLine.txt
    sed -i "${WRITECOUNTER} a ${TheLine}" ./r3bdata/R3BDataLinkDef.h
    WRITECOUNTER=`expr ${WRITECOUNTER} + 1`
    rm -rf ./Safe/FileLine.txt
...
sed -i '185 a     else if (detId == kVETO_SEG)' ./r3bdata/R3BMCTrack.cxx
sed -i '186 a        return ((fNPoints & 0xC00000000) >> 34);' ./r3bdata/R3BMCTrack.cxx
sed -i '187 a     else if (detId == kNVETO_SEG)' ./r3bdata/R3BMCTrack.cxx
sed -i '188 a        return ((fNPoints & 0x3000000000) >> 36);' ./r3bdata/R3BMCTrack.cxx

sed -i '274 a     else if (iDet == kVETO_SEG)' ./r3bdata/R3BMCTrack.cxx
sed -i '275 a     {' ./r3bdata/R3BMCTrack.cxx
sed -i '276 a         fNPoints = (fNPoints & (~0xC00000000)) | (nPoints << 34);' ./r3bdata/R3BMCTrack.cxx
sed -i '277 a     }' ./r3bdata/R3BMCTrack.cxx
sed -i '278 a     else if (iDet == kNVETO_SEG)' ./r3bdata/R3BMCTrack.cxx
sed -i '279 a     {' ./r3bdata/R3BMCTrack.cxx
sed -i '280 a         fNPoints = (fNPoints & (~0x3000000000)) | (nPoints << 36);' ./r3bdata/R3BMCTrack.cxx
sed -i '281 a     }' ./r3bdata/R3BMCTrack.cxx
