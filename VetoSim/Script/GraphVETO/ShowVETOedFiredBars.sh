...
# Write the counter:
cd ../Source/Graphics/
sed -i "67 a Counter = $1;" ./VETOedFiredBars_Geometry.cpp
sed -i '69 d' ./VETOedFiredBars_Geometry.cpp

# Write the event:
sed -i "64 a Event = $2;" ./VETOedFiredBars_Geometry.cpp
sed -i '66 d' ./VETOedFiredBars_Geometry.cpp

# And now we can actually run the code:
rm -rf ./*core_dump*
root -l -q VETOedFiredBars_Geometry.cpp
