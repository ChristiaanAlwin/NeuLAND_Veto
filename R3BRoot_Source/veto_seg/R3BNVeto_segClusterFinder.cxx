...
// -----                Created 2fNveto_seg_ClusterTimeRadius2-2012 by D.Kresan                     -----
...
bool SortDigiByTime(const R3Bveto_segDigi *digi1, const R3Bveto_segDigi *digi2);
  ...
  : FairTask("R3BNVeto_segClusterFinder")
{
  ...
  fNveto_seg_UseSphericalRadius = kFALSE;
  fNveto_seg_DeBug = kFALSE;

  // Create array for storing the clusters
  fArrayCluster = new TClonesArray("R3BNVeto_segCluster");
}
// -----------------------------------------------------------------------------
...
// -----------------------------------------------------------------------------
R3BNVeto_segClusterFinder::R3BNVeto_segClusterFinder(Int_t verbose)
  : FairTask("R3BNVeto_segClusterFinder", verbose)
{
...
InitStatus R3BNVeto_segClusterFinder::Init()
{
  ...
  // Check pointer
  if(NULL == rootMgr) {
    cout << "-E- R3BNVeto_segClusterFinder::Init : illegal pointer to root manager."
	 << endl;
    // Stop here
    return kFATAL;
  }

  // Get pointer to VETO_SEG digi array
  fArrayDigi = (TClonesArray*) rootMgr->GetObject("veto_segDigi");
  // Check pointer
  if(NULL == fArrayDigi) {
    cout << "-E- R3BNVeto_segClusterFinder::Init : no veto_segDigi array." << endl;
    // Stop here
    return kFATAL;
  }

  // Register the output array (will be written to the output file)
  rootMgr->Register("NVeto_segCluster", "Digi clusters in Nveto_seg",
		    fArrayCluster, kTRUE);
  ...
  fNveto_seg_Nplanes = Integers->GetBinContent(2);
...
// -----------------------------------------------------------------------------
void R3BNVeto_segClusterFinder::Exec(Option_t *option)
{
  // Debug output:
  Bool_t debug = fNveto_seg_DeBug;  
  ...
  R3Bveto_segDigi *digi1;
  R3Bveto_segDigi *digi2;

  //loop over entries:  
  for (Int_t i = 0; i < nDigis; i++) 
  {
    digi1 = (R3Bveto_segDigi*) fArrayDigi->At(i);
  ...
  Int_t Nbars = 50*fNveto_seg_Nplanes;
  ...
  R3BNVeto_segCluster *first_cluster;
  R3BNVeto_segCluster *second_cluster;
  R3BNVeto_segCluster *end_cluster;
  Int_t oldPaddle;
  Int_t first_clusNo;
  Int_t second_clusNo;
  Bool_t NoNeighbours;
  ...
  if (debug) {std::cout << "Spatial Search radius = " << fNveto_seg_ClusterSpaceRadius << " [cm] | Time Search radius = " << fNveto_seg_ClusterTimeRadius << "[ns]\n";}
  ...
  {  
	...
	if (fNveto_seg_UseSphericalRadius)
        {   
	  rad = TMath::Sqrt((posx1-posx2)*(posx1-posx2) + (posy1-posy2)*(posy1-posy2) + (posz1-posz2)*(posz1-posz2));
        }
        
	// Check if neighbour:
	if ((rad<fNveto_seg_ClusterSpaceRadius)&&(TMath::Abs(post1-post2)<fNveto_seg_ClusterTimeRadius)&&(i!=k)) // if i=k it is the same poit --> pointless!
        {
	  // This is a neighbour. Now there basically are 4 different situations:
          ...
          if ((belongsToCluster[i]==-1)&&(belongsToCluster[k]==-1))
          {
              ...
              new ((*fArrayCluster)[nClusters]) R3BNVeto_segCluster(digi1->GetXX(), digi1->GetYY(), digi1->GetZZ(), digi1->GetTdc(),
              ...
              new ((*fArrayCluster)[nClusters]) R3BNVeto_segCluster(digi2->GetXX(), digi2->GetYY(), digi2->GetZZ(), digi2->GetTdc(),
          ...
          else if ((belongsToCluster[i]>-1)&&(belongsToCluster[k]==-1))
          {
              ...
              // update the cluster:
              first_cluster = (R3BNVeto_segCluster*) fArrayCluster->At(first_clusNo);
          ...
          else if ((belongsToCluster[i]>-1)&&(belongsToCluster[k]>-1)&&(belongsToCluster[i]!=belongsToCluster[k]))
          {
              ...
              // Do actual merging:
              first_cluster = (R3BNVeto_segCluster*) fArrayCluster->At(first_clusNo);
              second_cluster = (R3BNVeto_segCluster*) fArrayCluster->At(second_clusNo);
              ...
              end_cluster = (R3BNVeto_segCluster*) fArrayCluster->At(nClusters-1); // NOTE: Need n-1 since nClusters is not a C++ index!
	...
	if (fNveto_seg_UseSphericalRadius)
	  ...
	  rad = TMath::Sqrt((posx1-posx2)*(posx1-posx2) + (posy1-posy2)*(posy1-posy2) + (posz1-posz2)*(posz1-posz2));
        ...
        if ((rad<fNveto_seg_ClusterSpaceRadius)&&(TMath::Abs(post1-post2)<fNveto_seg_ClusterTimeRadius)&&(i!=k))
        {
            NoNeighbours = kFALSE;
        }
      }
    ...
    {
      first_cluster = (R3BNVeto_segCluster*) fArrayCluster->At(i);
  ...
  if(fVerbose) 
  {
    cout << "-I- R3BNVeto_segClusterFinder : found " << nClusters << " clusters." << endl;
  }
...
// -----------------------------------------------------------------------------
// NOTE: Why 200 MeV protons do not always have exactly 1 cluster:
// ==>   200 MeV protons roughly have a 25 cm penetration depth in Nveto_seg.
//       1) Some events have secondary interactions deeply inside Nveto_seg.
//          These interactions are about 2m behind the intial proton track.
//       2) Nveto_seg has gabs, which are NOT active material. If a proton
//          accidentally comes through one of the gabs, a bar is missing
//          from the trajectory and therefore there cannot and should not 
//          be a single cluster anymore.
//       3) Some protons can scatter in the air or in the VETO and create
//          a second track generating more clusters in Nveto_seg.
//       4) The search radius of 7.5 cm is too small to always ensure that
//          two successive hits are always neighbours. Without uniform smearing,
//          the z-distance is always 5.0 cm. But the x and y distances can be
//          gaussian with sqrt(2)*150 ps sigma. Hence one only has a 95% probability
//          that the digitizer hit is within +/- 2*sqrt(2)*0.15*14 = 5.9 cm
//          of the real hit. 2 for 2 sigma. sqrt(2) for adding gaussians of PMT 1 & 2
//          0.15 ns is time resolution and 14 cm/ns is the effective speed of\
//          light inside the scintillator. For a sperical radius one therefore
//          needs 7.7 cm to have a 0.95^2 = 90% probability to get 2 digis being
//          neighbours. The cubic radius has 5.9 cm sufficient for this.
//       5) In some cases (about 8-9% from visual inspection) there is simply
//          a more complex interaction then just Bethe-Bloch energy loss.
//          In these cases, one therefore expects (and also finds) more then
//          1 cluster.
// -----------------------------------------------------------------------------
...
// -----------------------------------------------------------------------------
void R3BNVeto_segClusterFinder::CreateHistograms()
{
  // Initialize control histograms
