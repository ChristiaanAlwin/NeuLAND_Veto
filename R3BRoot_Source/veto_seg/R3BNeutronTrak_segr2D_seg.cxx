...
#include "R3BNeutHt_seg.h"
#include "R3BNeutronTrak_segr2D_seg.h"

using std::cout;
using std::endl;
using std::ifstream;

ClassImp(R3BNeutronTrak_segr2D_seg)



Double_t c = 29.9792458;
Double_t gBeamBeta;
bool AuxSortClustersBeta(R3BNVeto_segCluster*, R3BNVeto_segCluster*);

// -----------------------------------------------------------------------------
R3BNeutronTrak_segr2D_seg::R3BNeutronTrak_segr2D_seg()
    : FairTask("R3B Nveto_seg Neutron Tracker")
    , f2DCutEnabled(kTRUE)
    , fNNeutrons(0)
{
...
// -----------------------------------------------------------------------------
void R3BNeutronTrak_segr2D_seg::SetParContainers()
{
  ...
  if ( fveto_segDigiPar ) {
      cout << "-I- R3Bveto_segDigitizer::SetParContainers() "<< endl;
      cout << "-I- Container R3Bveto_segDigiPar  loaded " << endl;
  }
}
// -----------------------------------------------------------------------------
...
// -----------------------------------------------------------------------------
InitStatus R3BNeutronTrak_segr2D_seg::Init()
{
  // Get input array 
  FairRootManager* ioman = FairRootManager::Instance();
  if ( ! ioman ) Fatal("Init", "No FairRootManager");
  fveto_segPoints = (TClonesArray*) ioman->GetObject("veto_segPoint");
  fveto_segMCTrack = (TClonesArray*) ioman->GetObject("MCTrack");
  fveto_segDigi = (TClonesArray*) ioman->GetObject("veto_segDigi");
  fveto_segFirstHits = (TClonesArray*) ioman->GetObject("veto_segFirstHits");
  fArrayCluster = (TClonesArray*) ioman->GetObject("NVeto_segCluster");

  // New structure created by the Neutron Tracker
  fNeutHt_segs = new TClonesArray("R3BNeutHt_seg");
  ioman->Register("veto_segNeutHt_segs", "Neutron Hits", fNeutHt_segs, kTRUE);
  ...
  PRIM_part=new R3BPrimPat_seg*[10];
  PRIM_prot=new R3BPrimPat_seg*[10];
  PRIM_frag=new R3BPrimPat_seg*[10];
  PRIM_gamma=new R3BPrimPat_seg*[10];  
...
// -----------------------------------------------------------------------------
void R3BNeutronTrak_segr2D_seg::ReadCalibrFile(char *name)
{
  ...
  // Control output
  cout << "-I- R3BNeutronTrak_segr2D_seg : calibration : " << endl;
...
// -----------------------------------------------------------------------------
void R3BNeutronTrak_segr2D_seg::Exec(Option_t* opt)
{
  if(0 == (eventNo%100)) {
    cout << "-I- R3BNeutronTrak_segr2D_seg : Event #: " << eventNo << "." << endl;
  }
  eventNo += 1;
  ...
  Int_t prim;
  Int_t particleID;
  for(Int_t it = 0; it < fveto_segMCTrack->GetEntriesFast(); it++) {
    aTrack1 = (R3BMCTrack*) fveto_segMCTrack->At(it);
    ...
    if(2112 == particleID) {
      //neutron
      PRIM_part[fNPrimNeut] = new R3BPrimPat_seg(aTrack1->GetPdgCode(),
					      aTrack1->GetPx()*1000.,
					      aTrack1->GetPy()*1000.,
					      aTrack1->GetPz()*1000.,
					      aTrack1->GetStartX(),
      ...
      //proton
      PRIM_prot[fNPrimProt] = new R3BPrimPat_seg(aTrack1->GetPdgCode(),
					      aTrack1->GetPx()*1000.,
					      aTrack1->GetPy()*1000.,
					      aTrack1->GetPz()*1000.,
					      aTrack1->GetStartX(),
      ...
      //gamma
      PRIM_gamma[fNPrimGamma] = new R3BPrimPat_seg(aTrack1->GetPdgCode(),
						aTrack1->GetPx()*1000.,
						aTrack1->GetPy()*1000.,
						aTrack1->GetPz()*1000.,
						aTrack1->GetStartX(),
      ...
      PRIM_frag[fNofFrag] = new R3BPrimPat_seg(aTrack1->GetPdgCode(),
					    aTrack1->GetPx()*1000.,
					    aTrack1->GetPy()*1000.,
					    aTrack1->GetPz()*1000.,
					    aTrack1->GetStartX(),
  ...
  for(Int_t id = 0; id < fveto_segDigi->GetEntries(); id++) {
    digi = (R3Bveto_segDigi*) fveto_segDigi->At(id);
    fEtot += digi->GetQdc();
...
// -----------------------------------------------------------------------------
Int_t R3BNeutronTrak_segr2D_seg::AdvancedMethod()
{
  // Loop over clusters
  R3BNVeto_segCluster *cluster;
  for(Int_t ic = 0; ic < fNofClusters; ic++) {
    // Get pointer to the cluster
    cluster = (R3BNVeto_segCluster*) fArrayCluster->At(ic);
...
*/
      ...
      new ((*fNeutHt_segs)[fNofTracks]) R3BNeutHt_seg(pos.X(),
						pos.Y(),
						pos.Z(),
						cluster->GetStartT());
      nOutput += 1;
...
// -----------------------------------------------------------------------------
void R3BNeutronTrak_segr2D_seg::SortClustersBeta()
{
  if(0 == fNofClusters) {
    return;
  }
  ...
  cluster = (R3BNVeto_segCluster*) fArrayCluster->At(0);
...
// -----------------------------------------------------------------------------
void R3BNeutronTrak_segr2D_seg::NextIteration(Int_t curIndex, R3BNVeto_segCluster *curClus)
{
...
// -----------------------------------------------------------------------------
Bool_t R3BNeutronTrak_segr2D_seg::IsElastic(R3BNVeto_segCluster *c1, R3BNVeto_segCluster *c2)
{
  Double_t x0=0.;
  Double_t y0=0.;
  Double_t z0=0.;
...
// -----------------------------------------------------------------------------
void R3BNeutronTrak_segr2D_seg::CalculateMassInv()
{
  R3Bveto_segFirstHits *fh = (R3Bveto_segFirstHits*) fveto_segFirstHits->At(0);
  TVector3 fhv[6];
  ...
  for (Int_t i = 0; i < fNeutHt_segs->GetEntriesFast(); i++) {
    ...
    Int_t index;
    Double_t dmin = 1e6;
    for(Int_t ip = 0; ip < fveto_segPoints->GetEntriesFast(); ip++) {
      point = (R3Bveto_segPoint*) fveto_segPoints->At(ip);
    ...
    point = (R3Bveto_segPoint*) fveto_segPoints->At(index);
    Int_t trackID = point->GetTrackID();
    R3BMCTrack *track = (R3BMCTrack*) fveto_segMCTrack->At(trackID);
    Int_t mothID = track->GetMotherId();
    Int_t ind;
    while(-1 != mothID) {
      track = (R3BMCTrack*) fveto_segMCTrack->At(mothID);
      ind = mothID;
...
// -----------------------------------------------------------------------------
void R3BNeutronTrak_segr2D_seg::CalculateExce()
{
  TVector3 mom1;
  TVector3 mom2;
  Double_t en_g = 0;
  ...
  for(Int_t i = 0; i < fNofTracks; i++) {
    R3BNVeto_segCluster *c1 = fVectorCluster.at(fTracks[i][0]);
    TVector3 pos;
    ...
    p[k] = new R3BPrimPat_seg(2112, momX, momY, momZ,
			   0., 0., 0., 0.,
			   1.0086649, mNeutron);
    k += 1;
  }
...
// -----------------------------------------------------------------------------
void R3BNeutronTrak_segr2D_seg::Reset()
{
  for(Int_t i = 0; i < fNPrimNeut; i++) {
    delete PRIM_part[i];
  }
...
// -----------------------------------------------------------------------------
void R3BNeutronTrak_segr2D_seg::Finish()
{
  hClusters1->Write();
...
// -----------------------------------------------------------------------------
void R3BNeutronTrak_segr2D_seg::UseBeam(Double_t beam_energy, Double_t beam_beta)
{
...
// -----------------------------------------------------------------------------
void R3BNeutronTrak_segr2D_seg::CreateHistograms()
{
...
bool AuxSortClustersBeta(R3BNVeto_segCluster *c1, R3BNVeto_segCluster *c2)
{
  TVector3 pos1;
  TVector3 pos2;
  c1->StartPosition(pos1);
