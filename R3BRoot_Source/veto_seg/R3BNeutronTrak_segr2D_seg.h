...
#ifndef R3BNEUTRONTRAK_SEGR2D_SEG_H
#define R3BNEUTRONTRAK_SEGR2D_SEG_H
...
class R3BNeutronTrak_segr2D_seg : public FairTask
{
 public:
  /** Default constructor **/  
  R3BNeutronTrak_segr2D_seg();
 ...
 private:
  ...
  TClonesArray* fveto_segFirstHits;
  TClonesArray* fNeutHt_segs;
  ...
  Double_t calFactor; //calibration factor energy of VETO_SEG paddles
  Int_t eventNo;
  R3BPrimPat_seg **PRIM_part;
  R3BPrimPat_seg **PRIM_prot;
  R3BPrimPat_seg **PRIM_frag;
  R3BPrimPat_seg **PRIM_gamma;
  ...
  std::vector<R3BNVeto_segCluster*> fVectorCluster;
  Int_t fNofClusters1;
  Int_t fNofTracks;
  ...
  void NextIteration(Int_t curIndex, R3BNVeto_segCluster *curClus);
  Bool_t IsElastic(R3BNVeto_segCluster *c1, R3BNVeto_segCluster *c2);

 public: 
  ClassDef(R3BNeutronTrak_segr2D_seg,1)  
};
