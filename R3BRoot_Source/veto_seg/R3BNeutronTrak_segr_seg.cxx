#include "R3BNeutronTrak_segr_seg.h"
...
R3BNeutronTrak_segr_seg::R3BNeutronTrak_segr_seg() :
  FairTask("R3B veto_seg Digitization scheme ") { 
}
...
void R3BNeutronTrak_segr_seg::SetParContainers() {
...
InitStatus R3BNeutronTrak_segr_seg::Init() {
  ...
  fNeutronTrak_segs = new TClonesArray("R3BNeutronTrak_seg");
  ioman->Register("veto_segNeTracks", "Neutron Tracks", fNeutronTrak_segs, kTRUE);
...
// -----   Public method Exec   --------------------------------------------
void R3BNeutronTrak_segr_seg::Exec(Option_t* opt) {
...
//-Reset local arrays 
   ...
   Int_t nentries = fveto_segDigi->GetEntries();
...
//   Double_t betaNeutron,E_lab,pnzcm;
   ...
   R3BMCTrack *aTrack1 = (R3BMCTrack*) fveto_segMCTrack->At(0);
...
//	 cout<<"prim M "<<PRIM_frag[0].M<<endl;
	 cout<<"Mass "<<PRIM_frag[0].M/0.92827231<<endl;
      ...
      nPrim=nPrim+1;
      aTrack1 = (R3BMCTrack*) fveto_segMCTrack->At(nPrim);
   ...
   for (Int_t l=0;l<nentries;l++){
      ...
      // Get the veto_seg Object in array
      R3Bveto_segDigi *veto_seg_obj = (R3Bveto_segDigi*) fveto_segDigi->At(l);


      temp[l][0] = int(veto_seg_obj->GetPaddleNr())-1; //note that paddle starts at 1 
      temp[l][1] = veto_seg_obj->GetTdcL();  
      temp[l][2] = veto_seg_obj->GetTdcR();  
      temp[l][3] = veto_seg_obj->GetTdc();  
      temp[l][4] = veto_seg_obj->GetQdcL();  
      temp[l][5] = veto_seg_obj->GetQdcR();  
      temp[l][6] = veto_seg_obj->GetQdc();  
      temp[l][7] = veto_seg_obj->GetXX();  
      temp[l][8] = veto_seg_obj->GetYY();  
      temp[l][9] = veto_seg_obj->GetZZ(); 
...
//   Int_t nentr = fveto_segFirstHits->GetEntries();

//   cout<<"entries: "<<nentr<<endl;
   R3Bveto_segFirstHits *veto_seg_obj1 = (R3Bveto_segFirstHits*) fveto_segFirstHits->At(0);
   
   firstHitX[0] = veto_seg_obj1->GetX0();  
   firstHitY[0] = veto_seg_obj1->GetY0();  
   firstHitZ[0] = veto_seg_obj1->GetZ0();  
   firstT[0] = 1.E9*veto_seg_obj1->GetT0();  
   firstHitX[1] = veto_seg_obj1->GetX1();  
   firstHitY[1] = veto_seg_obj1->GetY1();  
   firstHitZ[1] = veto_seg_obj1->GetZ1();  
   firstT[1] = 1.E9*veto_seg_obj1->GetT1();  
   firstHitX[2] = veto_seg_obj1->GetX2();  
   firstHitY[2] = veto_seg_obj1->GetY2();  
   firstHitZ[2] = veto_seg_obj1->GetZ2();  
   firstT[2] = 1.E9*veto_seg_obj1->GetT2();  
   firstHitX[3] = veto_seg_obj1->GetX3();  
   firstHitY[3] = veto_seg_obj1->GetY3();  
   firstHitZ[3] = veto_seg_obj1->GetZ3();  
   firstT[3] = 1.E9*veto_seg_obj1->GetT3();  

   firstHitX[4] = veto_seg_obj1->GetX4();  
   firstHitY[4] = veto_seg_obj1->GetY4();  
   firstHitZ[4] = veto_seg_obj1->GetZ4();  
   firstT[4] = 1.E9*veto_seg_obj1->GetT4();  
   firstHitX[5] = veto_seg_obj1->GetX5();  
   firstHitY[5] = veto_seg_obj1->GetY5();  
   firstHitZ[5] = veto_seg_obj1->GetZ5();  
   firstT[5] = 1.E9*veto_seg_obj1->GetT5();  
...
/*
//      TrackId = veto_seg_obj->GetTrackID();
      Int_t TrackId=0;
      Int_t VolumeID=0;
      for (Int_t i=0;i<Nclusters;i++){         
...
void R3BNeutronTrak_segr_seg::Reset(){
// Clear the structure
//   cout << " -I- Digit Reset() called " << endl;
// if (fDigis ) fDigis->Clear();
}   

void R3BNeutronTrak_segr_seg::Finish()
{
...
R3BNeutronTrak_seg* R3BNeutronTrak_segr_seg::AddHit(TVector3 posIn,
			TVector3 posOut, TVector3 momOut, Double_t time){
  // It fills the R3Bveto_segDigi array
  TClonesArray& clref = *fNeutronTrak_segs;
  Int_t size = clref.GetEntriesFast();
  return new(clref[size]) R3BNeutronTrak_seg(posIn, posOut, momOut, time);
}

// -----  Public method UseBeam  ----------------------------------
void R3BNeutronTrak_segr_seg::UseBeam(Double_t beam_energy, Double_t beam_beta)
{
   beamEnergy   = beam_energy;
   beamBeta   = beam_beta;
}

ClassImp(R3BNeutronTrak_segr_seg)
