...
#include "R3Bveto_segDigitizer.h"

const Double_t R3Bveto_segDigitizer::DEFAULT_SATURATION_COEFFICIENT = 0.012;
...
inline Double_t R3Bveto_segDigitizer::BuildTOFRangeFromBeamEnergy(const Double_t &e) // [ns]
{
  if(e > 1100) {
    return 15.;
  }
...
// ----------------------------------------------------------------------------
R3Bveto_segDigitizer::R3Bveto_segDigitizer()
  : FairTask("R3B veto_seg Digitization scheme"),
    f1(new TF1("f1", fun1, 0., 1000., 2)),
    fRnd(new TRandom3()),
    fThreshFileName(""),
    fNChannels(0),
    fSaturationCoefficient(R3Bveto_segDigitizer::DEFAULT_SATURATION_COEFFICIENT)
{
}
// ----------------------------------------------------------------------------
...
// ----------------------------------------------------------------------------
R3Bveto_segDigitizer::R3Bveto_segDigitizer(Int_t verbose)
  : FairTask("R3B veto_seg Digitization scheme ", verbose),
    f1(new TF1("f1", fun1, 0., 1000., 2)),
    fRnd(new TRandom3()),
    fThreshFileName(""),
    fNChannels(0),
...
// ----------------------------------------------------------------------------
void R3Bveto_segDigitizer::SetParContainers()
{
  ...
  if ( fVerbose && fveto_segDigiPar ) {
    LOG(INFO) << "R3Bveto_segDigitizer::SetParContainers() "<< FairLogger::endl;
...
// ----------------------------------------------------------------------------
InitStatus R3Bveto_segDigitizer::Init()
{
  // Get input array
  ...
  // Register output array veto_segDigi
  fveto_segDigi = new TClonesArray("R3Bveto_segDigi",1000);
  ...
  LOG(INFO) << "R3Bveto_segDigitizer: # paddles: " << npaddles-1
  << ", # planes: " << nplanes << FairLogger::endl;
  LOG(INFO) << "R3Bveto_segDigitizer: paddles per plane: " << paddle_per_plane << FairLogger::endl;
  c = 2.99792458E8;
...
// -----   Public method Exec   --------------------------------------------
void R3Bveto_segDigitizer::Exec(Option_t* opt)
{
  ...
  plength = fveto_segDigiPar->GetPaddleLength(); // [cm]

  Double_t Thresh = 0.16;
//  Double_t Thresh = 0.05;


  Int_t nentries = fveto_segPoints->GetEntries();
  ...
  for (Int_t l=0; l < nentries; l++) {
    R3Bveto_segPoint *veto_seg_obj = (R3Bveto_segPoint*) fveto_segPoints->At(l);

    Int_t paddle = int(veto_seg_obj->GetSector())-1; //note that paddle starts at 1
    Int_t scint = int(veto_seg_obj->GetPaddleNb());
    Double_t eloss = veto_seg_obj->GetEnergyLoss()*1000.;
    Double_t light = veto_seg_obj->GetLightYield()*1000.;
    Double_t x = veto_seg_obj->GetXIn();
    Double_t y = veto_seg_obj->GetYIn();
    Double_t z = veto_seg_obj->GetZIn();
    Double_t time = veto_seg_obj->GetTime();
    Int_t media = int(veto_seg_obj->GetPaddleType());
    ...
    if (eloss > 0. && media == 3) {
      ...
      if (m>1500) {
        LOG(ERROR) << "R3Bveto_segDigitizer: Too many hits in one paddle: "<< m << " hits" << FairLogger::endl
...
//      }
      ...
      // Here is an example how to fill the R3Bveto_segDigi structure
      Double_t tdcL = tofl;
      Double_t tdcR = tofr;
  ...
  if(fVerbose) {
    LOG(INFO) << "R3Bveto_segDigitizer: produced "
...
// ----------------------------------------------------------------------------
void R3Bveto_segDigitizer::Reset()
{
  if (fveto_segDigi) {
    fveto_segDigi->Clear();
  }
...
// ----------------------------------------------------------------------------
void R3Bveto_segDigitizer::Finish()
{
  hPMl->Write();
  hPMr->Write();
  hMult1->Write();
...
// ----------------------------------------------------------------------------
R3Bveto_segDigi* R3Bveto_segDigitizer::AddHit(Int_t paddleNr, Double_t tdcL, Double_t tdcR,
                                      Double_t tdc, Double_t qdcL, Double_t qdcR, Double_t qdc,
                                      Double_t xx, Double_t yy, Double_t zz)
{
  R3Bveto_segDigi *digi = new((*fveto_segDigi)[fveto_segDigi->GetEntriesFast()]) R3Bveto_segDigi(paddleNr,
                                                                                 tdcL, tdcR, tdc,
                                                                                 qdcL, qdcR, qdc,
                                                                                 xx, yy, zz);
  return digi;
