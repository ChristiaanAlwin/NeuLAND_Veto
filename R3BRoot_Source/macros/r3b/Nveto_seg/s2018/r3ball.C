            ...
            TString InFile = "evt_gen.dat")
{
  ...
  // veto_seg Detector
  if (fDetList->FindObject("VETO_SEG") ) {
    R3BDetector* veto_seg = new R3Bveto_seg("veto_seg", kTRUE);
    veto_seg->SetVerboseLevel(1);
    veto_seg->SetGeometryFileName(((TObjString*)fDetList->GetValue("VETO_SEG"))->GetString().Data());
...
//    boxGen->SetXYZ(0.0, 0.0, -300.);
  ...
  if (fGenerator.CompareTo("ascii") == 0  ) {
    R3BAsciiGenerator* gen = new R3BAsciiGenerator((dir+"/input/Nveto_seg/event_600AMeV_500keV/"+InFile).Data());
