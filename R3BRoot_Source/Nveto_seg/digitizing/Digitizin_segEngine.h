#ifndef _NVETO_SEG_DIGITIZIN_seg_ENGINE_H_
#define _NVETO_SEG_DIGITIZIN_seg_ENGINE_H_

/* Nveto_seg Digitizin_seg Engine
 * @author: Jan Mayer
 *
 * The Nveto_seg Digitizin_seg Engine (DE) handles the detector physics and data acquisition
 ...
 * the actual conversion from `veto_segPoints` to `veto_segDigis`, is handled by tasks, e.g. by
 * the R3BNveto_segDigitizer.
 *
 *
 * All scintillator bars in the Nveto_seg detector are equipped a photomultiplier (PMT) on
...
#include "Validatd_seg.h"

namespace Nveto_seg {

class Digitizin_segEngine {
...
static Double_t fAttenuation;
static Double_t fcMedium;
    struct PMTHit {
        Double_t time;
        Double_t light;
    ...
    private:
        ...
        mutable Validatd_seg<std::vector<PMTHit>::const_iterator> cachedFirstHitOverThresh;
        mutable Validatd_seg<Double_t> cachedQDC;
        mutable Validatd_seg<Double_t> cachedTDC;
        mutable Validatd_seg<Double_t> cachedEnergy;
...
#endif //_NVETO_SEG_DIGITIZIN_seg_ENGINE_H_
